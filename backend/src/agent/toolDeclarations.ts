...
export const TOOL_DECLARATIONS: FunctionDeclaration[] = [
  ...
  {
    name: 'google_search',
    description: 'Perform a live Google Search to find recent news, facts, articles, or documentation. Use when asked "Search the web for X", "What is the latest on Y", or "Research Z".',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        query: { type: SchemaType.STRING, description: 'The search query to execute.' },
      },
      required: ['query'],
    },
  },
  {
    ...
    description: 'Fetch and extract the main markdown content from any live URL. Use to summarize links, read articles, or parse specific web pages.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        url: { type: SchemaType.STRING, description: 'The exact URL to fetch (must include http/https).' },
      },
      required: ['url'],
    },
  },
  {
    ...
    description: 'Register a price tracking monitor for a specific product URL. Alerts the user when the price drops below the target price.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        url: { type: SchemaType.STRING, description: 'The URL of the product page to monitor.' },
        targetPrice: { type: SchemaType.NUMBER, description: 'The numeric target price threshold to trigger the alert.' },
      },
      required: ['url', 'targetPrice'],
    },
  },
  ...
  // ─── GYM & FITNESS MODULE ─────────────────────────────────────────────────
  {
    ...
    description: 'Records sets, reps, and weight for a completed gym session directly to the live workout log.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        date: { type: SchemaType.STRING, description: 'The date of the session in YYYY-MM-DD format (use today if unspecified)' },
        exerciseName: { type: SchemaType.STRING, description: 'The name of the exercise (e.g., "Bench Press", "Squat")' },
        sets: { 
          type: SchemaType.ARRAY, 
          description: 'Array of objects containing {reps, weight} for each completed set',
          items: {
            type: SchemaType.OBJECT,
            properties: {
              reps: { type: SchemaType.NUMBER, description: 'Number of reps completed' },
              weight: { type: SchemaType.NUMBER, description: 'Weight used (in kg)' }
            }
          }
        },
      },
      required: ['date', 'exerciseName', 'sets'],
    },
  },
  {
    name: 'get_progressive_overload_suggestion',
    description: 'Computes next session\'s target weights by finding the last time the user performed this exercise and adding 2.5kg if they hit their reps.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        exerciseName: { type: SchemaType.STRING, description: 'The exact name of the exercise to analyze' },
      },
      required: ['exerciseName'],
    },
  },
  {
    name: 'get_gym_weekly_summary',
    description: 'Fetches muscle groups, total volume, and frequency for the past 7 days to analyze overtraining or progress.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        targetDate: { type: SchemaType.STRING, description: 'The anchor date to look back 7 days from in YYYY-MM-DD format (defaults to today)' },
      },
      required: [],
    },
  },
