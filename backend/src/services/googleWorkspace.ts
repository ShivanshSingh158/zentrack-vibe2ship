...
// MISSING-009: Gmail Vacation Responder (Auto-Reply) API
// Previously focus_lock CLAIMED to set an auto-reply but never called the Gmail API.
// These functions call users.settings.updateVacationSettings to set a real auto-reply.

export interface GmailVacationSettings {
  enabled: boolean;
  responseSubject?: string;
  responseBodyPlainText?: string;
  /** Unix milliseconds — start of the auto-reply window */
  startTimeMs?: number;
  /** Unix milliseconds — end of the auto-reply window */
  endTimeMs?: number;
  /** If true, only send to people in your Contacts */
  restrictToContacts?: boolean;
  /** If true, only send to people in your Google Workspace domain */
  restrictToDomain?: boolean;
}

export const setGmailVacationResponder = async (settings: GmailVacationSettings, signal?: AbortSignal): Promise<void> => {
  await workspaceFetch<void>(
    `${GMAIL_API}/settings/vacation`,
    'PUT',
    {
      enableAutoReply: settings.enabled,
      responseSubject: settings.responseSubject || 'Currently in Deep Focus Mode',
      responseBodyPlainText: settings.responseBodyPlainText || 'I\'m currently in a deep focus session and will respond when I\'m done.',
      startTime: settings.startTimeMs?.toString(),
      endTime: settings.endTimeMs?.toString(),
...
export const clearGmailVacationResponder = async (signal?: AbortSignal): Promise<void> => {
  await workspaceFetch<void>(
    `${GMAIL_API}/settings/vacation`,
    'PUT',
