...
└── src/
    ...
    └── utils/
        ...
        ├── timeUtils.ts                  # Shared Time Parsing, Range Formatting & Deduplicating Sanitizer
...
|---|---|---|---|
| [`src/screens/DashboardScreen.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/DashboardScreen.tsx) | `DashboardScreen` | `Home` | Main Dashboard: Life Matrix ring, habit streak rings, hydration logger, Voice Task Dictation FAB, streamlined iOS 18 glass header with Streak capsule, theme quick-toggle, and user avatar triggering authentic Apple iOS 18 Inset Grouped `BottomSheet` Quick Profile Sheet (hero card with accent avatar ring, display name, rank badge with chevron, real-time XP progress track with percentage fill, uppercase section headers, customize dashboard action, theme switch pill with Sun/Moon icons, streak counter badge, and All Settings launcher). |
...
| [`src/screens/AttendanceScreen.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/AttendanceScreen.tsx) | `AttendanceScreen` | `Attendance` | Attendance Tracker: subject card list with Apple 20px squircle curvature, health-tinted percentage pills, vertically aligned progress tracks (Class & Lab), staggered spring entrance cascade (FadeInDown on daily sessions & by-subject cards), clean bunk margin status callouts; Universal Frame-0 spring touch physics via `AnimatedPressable` across Present/Absent/Cancelled status chips (`variant="button"` with synchronized status haptics), tactile header action buttons (Due, Holiday, Alerts, Setup), unlogged session rows, and subject cards; un-guarded BottomSheet and Modal rendering allowing seamless 60/120fps iOS slide-out and spring dismiss transitions; danger zone banner; timetable grid; Excel import/export; and chronological Subject History modal. |
...
| [`src/screens/HabitsScreen.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/HabitsScreen.tsx) | `HabitsScreen` | `Habits` | HabitKit-Tier Habit Command Center: 35-day GitHub contribution heatmaps per card, squircle tiles with 4-tier graduated luminance, cyan freeze highlights, interactive historical date tooltip HUD, segmented filter bar (`All`, `Building`, `Avoiding`), 365-day panoramic annual analytics modal with day-of-week radar and 4-pill KPI summary, zero layout-shift skeleton matrix, WhatsApp-grade offline safeWrite sync, and Universal Frame-0 spring touch physics via `AnimatedPressable` on habit title pressables, daily check-off action rings (`variant="button" haptic="success"` for instant zero-lag tactile completion ticks), and 7-day strip column toggles. |
| [`src/screens/NotesScreen.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/NotesScreen.tsx) | `NotesScreen` | `Notes` | ZenNotes: Markdown editor, hierarchical file/folder storage nodes, AI co-writer assistance, PDF exporter, Cloud Vault with FileSystem size probing, two-stage adaptive PDF compression (recommended → extreme) via iLovePDF, Cloudinary 10.0 MB gate, detailed diagnostic error alerts, and Universal Frame-0 spring touch physics via `AnimatedPressable` across header navigation controls (Back, Cancel, Done, Search, Sort, Select), breadcrumb navigation pills, search controls, and `StorageNodeRow` items. |
...
| [`src/screens/tasks/NewTaskModal.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/tasks/NewTaskModal.tsx) | `NewTaskModal` | Slide-up modal for task creation with NLP natural language parsing chips, cleanly splitting parsed ranges into discrete `startTime` and `endTime` states to prevent duplicated range concatenation (`start - end - end`). |
| [`src/screens/tasks/EditTaskModal.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/tasks/EditTaskModal.tsx) | `EditTaskModal` | Apple Reminders iOS 18 "Grouped Inspector" modal: top nav bar ("Cancel", "Details", "Done"); Group 1 (standalone NLP task title with fast 60ms real-time token extraction for Date, Time, Priority, Recurrence; eliminated extraneous notes input); Group 2 Schedule (Date with 1-tap shortcuts Today/Tomorrow/Weekend, structured Time Slot with dual Starts/Ends cards and 1-tap Clear, notification Switch, recurrence picker, with clean discrete `startTime` and `endTime` extraction preventing duplicated range concatenation); Group 3 (Priority segmented control None/Low/Med/High + conditional Tag selector showing active tags only when present, eliminated suggested tags clutter); Group 4 (Checkable Subtasks with progress counter and chained inline "+ Add step" input); and bottom Save CTA + red Delete Task button (eliminated focus duration block). |
...
| [`src/screens/dashboard/useDashboardData.ts`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/dashboard/useDashboardData.ts) | `useDashboardData()` | Granular domain coordinator for Dashboard: sub-indexes CoreData, Academic, Wellness, and Planner slices, exports `optimisticUpdateTask` for 0ms dashboard task checking, quotes, layout, and life rings. |
| [`src/screens/dashboard/dashboardStyles.ts`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/dashboard/dashboardStyles.ts) | `makeStyles(colors)` | Dynamic style factory for DashboardScreen, including Apple iOS 18 Inset Grouped Quick Profile BottomSheet (hero squircle card with avatar ring, XP progress bar, uppercase section headers, 20px squircle group cards, 34x34 tinted icon boxes, theme pill with Sun/Moon icons, streak badge, and indented hairline dividers). |
|
...
| [`src/components/AnimatedPressable.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/components/AnimatedPressable.tsx) | `AnimatedPressable` | Universal UI-thread Reanimated worklet pressable running with zero touch latency (`unstable_pressDelay={0}` to bypass React Native scroll gesture delays), semantic press depths (`row: 0.985`, `card: 0.980`, `button: 0.960`, `cta: 0.930`, `destructive: 0.940`, `subtle: 0.975`), critically damped UI-thread spring physics (`SPRING_IN`: stiffness 450, damping 18, mass 0.35, overshootClamping=true; `SPRING_OUT`: stiffness 350, damping 15, mass 0.45), and synchronized micro-haptics on touch-down (`light`, `selection`, `medium`, `heavy`, `success`). Powers primary buttons, attendance status chips, habit action checks, and vault items across AttendanceScreen, HabitsScreen, NotesScreen, and StorageNodeRow. |
