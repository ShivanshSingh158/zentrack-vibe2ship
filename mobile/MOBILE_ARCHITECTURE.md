...
| [`src/screens/dashboard/dashboardStyles.ts`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/screens/dashboard/dashboardStyles.ts) | `makeStyles(colors)` | Dynamic style factory for DashboardScreen, including iOS 18 Quick Profile BottomSheet grouped cards, rows, icons, and theme pills. |
|
...
| [`src/components/Dashboard/AgendaWidget.tsx`](file:///c:/Users/perso/.gemini/antigravity/scratch/zentrack-vibe2ship/mobile/src/components/Dashboard/AgendaWidget.tsx) | `AgendaWidget` | Apple iOS 18 Inset Grouped Widget for Today's schedule on Home dashboard: squircle container (`borderRadius: 20`, hairline border, subtle shadow), uppercase header with dynamic count badge pill (`X Left` / `All Done ✓`), category squircle tags (Academic, Gym, Task), indented dividers (`marginLeft: 50`), direct 1-tap interactive circular checkboxes for instant task completion with optimistic UI and haptic feedback, celebratory empty state with quick action chips (`+ Add task` and `🎙 Voice`), and automatic holiday suppression. |
