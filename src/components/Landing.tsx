...
export const Landing = ({ onTryNow }: { onTryNow: () => void }) => {
  ...
  const videoOpacity = useTransform(smooth, [0, 0.45], [1, 0.5]);
  ...
  return (
    <div className="landing-page" ref={containerRef}>
      ...
      {/* ── Video background ──────────────────────────── */}
      <motion.video autoPlay loop muted playsInline src={BG_VIDEO}
        className="landing-video" style={{ scale: videoScale, opacity: videoOpacity }} />

      {/* ── Dark gradient overlay ─────────────────────── */}
      <div style={{ position: 'fixed', inset: 0, zIndex: 1, pointerEvents: 'none', background: 'linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.08) 35%, rgba(0,0,0,0.65) 100%)' }} />
