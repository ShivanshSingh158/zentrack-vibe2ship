...
function useAmbientSound(volumeLevel: SoundLevel) {
  ...
  useEffect(() => {
    if (!audioRef.current) {
      const audio = new Audio('/ambient.webm');
      audio.loop = true;
    ...
    // Set volume (scaled down slightly because webm might be loud)
    audio.volume = volumeLevel === 'high' ? 0.40 : 0.15;

    const tryPlay = () => {
      if (volumeLevel !== 'off' && !document.hidden) {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        audio.pause();
      } else if (volumeLevel !== 'off') {
        tryPlay();
      }
    };

    // Need interaction to unlock audio on some browsers
    ...
    <div className="landing-page" ref={containerRef}>
      ...
      <motion.div className="landing-hero" style={{ y: heroY, minHeight: '80vh', paddingTop: '4rem', paddingBottom: '3.5rem' }}>
        ...
        <motion.div style={{ display: 'flex', gap: '1rem', marginTop: '3rem', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}
          ...
          <motion.a
            href="/ZenTrack.apk"
            download="ZenTrack.apk"
            className="landing-download-btn premium-glow"
          ...
          <div style={{ maxWidth: '52rem', margin: '0 auto' }}>
            ...
            {isMobile ? (
              <motion.a
                href="/ZenTrack.apk"
                download="ZenTrack.apk"
                className="hero-cta-btn premium-matte-card premium-glow"
                whileHover={{ scale: 1.04 }}
                whileTap={{ scale: 0.97 }}
                style={{
                  marginTop: 0,
              ...
              >
                ...
                <span>Download ZenTrack for Android</span>
              </motion.a>
            ) : (
              <motion.button onClick={onTryNow} className="hero-cta-btn premium-matte-card premium-glow"
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
