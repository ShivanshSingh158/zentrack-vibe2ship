/**
 * AttendanceScreen ΓÇö ZenTrack Mobile
...
// ΓöÇΓöÇΓöÇ Constants & Helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const calculateStatus = (attended: number, total: number, target: number) => {
  ...
  if (bunkResult.status === 'safe') {
    ...
    bunkInfo = `Γ£ô ${bunkResult.message.replace('You can safely bunk', 'Can miss').replace(' and stay above ' + target + '%', '')}`;
  } else if (bunkResult.status === 'warning') {
    urgency = 'warning';
    bunkInfo = 'ΓÜá∩╕Å On the edge ΓÇö 0 misses left';
  } else {
    urgency = 'danger';
    bunkInfo = `ΓÜá∩╕Å Must attend next ${bunkResult.count} classes`;
  }
...
export default function AttendanceScreen() {
  ...
  // ΓöÇΓöÇ Schema Migration ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Load Logs & Holidays ΓöÇΓöÇ
  useEffect(() => {
    if (!user) return;
    ...
    todayScheduledSubjects.forEach(subject => {
      ...
      for (let i = 0; i < classCount; i++) {
        ...
        const timeStr = session?.time
          ? [session.time, session.room].filter(Boolean).join(' ΓÇó ')
          : `Class #${i + 1}`;
        sessions.push({
    ...
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 16, paddingTop: 8, paddingBottom: 16 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10 }}>
          ...
          {/* Holiday Toggle ΓÇö fixed icon button, amber when active */}
          <TouchableOpacity
          ...
          >
            <Text style={{ fontSize: 15 }}>≡ƒî┤</Text>
          </TouchableOpacity>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            {/* ΓöÇΓöÇ Semester Overview ΓöÇΓöÇ */}
                  ...
                  {warningSubjects.map(s => {
                    ...
                    return (
                      <Text key={s.id} style={{ fontSize: 11, color: '#fca5a5' }}>
                        ΓÇó <Text style={{ fontWeight: 'bold' }}>{s.name}</Text>: {pct}% ΓÇö attend {need} more to recover
                      </Text>
                    )
                  })}
                </View>
              ...
              {weekDates.map((date, i) => {
                ...
                return (
                  <TouchableOpacity key={date} onPress={() => setSelectedDate(date)} style={[{ alignItems: 'center', paddingVertical: 4, paddingHorizontal: 8 }, isSel && { backgroundColor: colors.accentPrimary, borderRadius: 10 }]}>
                    ...
                    <Text style={{ fontSize: 13, color: isSel ? '#000000' : colors.textTertiary, fontWeight: isSel ? '600' : '400' }}>{isHol ? '≡ƒî┤' : date.split('-')[2]}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>

            {/* ΓöÇΓöÇ Daily Schedule ΓöÇΓöÇ */}
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>{isSelectedHoliday ? "It's a Holiday! ≡ƒî┤" : "No classes scheduled!"}</Text>
            {subjects.length === 0 && (
                ...
                {log ? (
                  ...
                  >
                    <Text style={{
                      ...
                      {log.action === 'attended' ? 'Present' : log.action === 'cancelled' ? 'Cancelled' : 'Absent'} Γå⌐
                    </Text>
                  </TouchableOpacity>
                ) : (
                  <>
              ...
              {todayScheduledSubjects.map(subject => {
                ...
                return (
                  <View key={subject.id}>
                    <TouchableOpacity onPress={() => setSelectedHistorySubject(subject)} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', paddingVertical: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: colors.border }}>
                      ...
                      <View style={{ flex: 1, paddingRight: 8 }}>
                        ...
                        {hasLabs ? (
                          ...
                          // No labs ΓÇö show bunk budget pill
                          (() => {
                            ...
                            const budgetLabel = bunk.status === 'safe' && bunk.count > 0
                              ? `Γ£ô Can miss ${bunk.count} more`
                              : bunk.status === 'warning'
                              ? `ΓÜá 0 misses left`
                              : `Γåæ Attend ${bunk.count} to recover`;
                            return (
                    ...
                    {/* Inline recovery hint ΓÇö no formula, no box */}
                    {(() => {
              ...
              <View style={styles.historyCard}>
                <View>
                  <Text style={{ fontFamily: FONT_FAMILY.bold, fontSize: 14, color: l.action === 'attended' ? '#10b981' : '#ef4444' }}>
                    {l.action === 'attended' ? 'Γ£ô Attended' : 'Γ£ù Missed'} <Text style={{ color: colors.textPrimary }}>{l.isExtra ? '(Extra) ' : ''}{l.type||'class'}</Text>
                  </Text>
                  <Text style={{ fontSize: 11, color: colors.textMuted, marginTop: 4 }}>{formatDisplayDate(l.date)} ΓÇó {new Date(l.timestamp).toLocaleTimeString([], { hour:'2-digit', minute:'2-digit' })}</Text>
                </View>
      ...
      <Modal visible={isExtraOpen} transparent animationType="slide">
        <KeyboardAvoidingView style={styles.overlayBg} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <View style={styles.sheet}>
            ...
            {/* Subject selector ΓÇö vertical full-width pills */}
...
const makeStyles = (colors: any) => StyleSheet.create({
      ...
      // Extra Class Modal ΓÇö redesigned
      subjectSelectRow: {
        flexDirection: 'row',
        alignItems: 'center',
        gap: 10,
